...
def _attach_warning_capture(*logger_names: str):
    """Capture WARNING+ records from named loggers; returns (records, detach)."""
    import logging
...
def repro_search_current(skill_dir: Path) -> dict:
    ...
    warn_records, detach_warn = _attach_warning_capture("xskill", "xskill.server")
    obj = SimpleNamespace(
    ...
    # Index present but embedding unset: still skip embed client + warn.
    (skill_dir / ".skill_index.pkl").write_bytes(b"placeholder")
    resp_unset = client.post(
        "/api/v1/skills/search", json={"query": "heartbeat", "top_k": 2},
    )
    print(
        f"POST /api/v1/skills/search (index+unset embed) "
        f"status={resp_unset.status_code} body={resp_unset.text[:400]}"
    )
    (skill_dir / ".skill_index.pkl").unlink(missing_ok=True)
    detach_warn()
    print("warnings:")
    for message in warn_records:
        print(f"  - {message}")

    search_warned = any(
        "skill search skipped" in message and ".skill_index.pkl" in message
        for message in warn_records
    )
    resolve_warned = any("skill resolve skipped" in message for message in warn_records)
    unset_embed_warned = any(
        "embedding.base_url/model unset" in message for message in warn_records
    )
    return {
        ...
        "api_unset_embed_status": resp_unset.status_code,
        "api_unset_embed_body": resp_unset.text[:400],
        "warnings": warn_records,
        "search_warned": search_warned,
        "resolve_warned": resolve_warned,
        "unset_embed_warned": unset_embed_warned,
    }
...
def main() -> int:
    ...
    if args.mode == "current":
        ...
        unset_ok = se.get("api_unset_embed_status") == 200
        warn_ok = bool(
            se.get("search_warned")
            and se.get("resolve_warned")
            and se.get("unset_embed_warned")
        )
        ...
        print("api_unset_embed_ok=", unset_ok)
        print("search_warned=", se.get("search_warned"))
        print("resolve_warned=", se.get("resolve_warned"))
        print("unset_embed_warned=", se.get("unset_embed_warned"))
        print("warn_ok=", warn_ok)
        return 0 if (
            status_ok and attr_ok and search_ok and resolve_ok and unset_ok and warn_ok
        ) else 1
